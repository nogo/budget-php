(function (global, factory) {
	        ...
	        m('div.row',
	            m('.col.s12', [
	                ...
	                m('input#date', {
	                    ...
	                    name: 'date',
	                    onchange: scope.update,
	                    value: moment$1(scope.item.date).format('YYYY-MM-DD')
	                })
	            ])
	        ),
	        m('.row', [
	...
	var numeral = __commonjs(function (module, exports, global) {
	/*!
	 * numeral.js
	 * version : 1.5.3
	 * author : Adam Draper
	...
	var numeral$1 = (numeral && typeof numeral === 'object' && 'default' in numeral ? numeral['default'] : numeral);
	...
	function budgetItemView(title, item, currentDate) {
	    ...
	    if (item) {
	        ...
	        if (item.amount) {
	            result.push(m('span.secondary-content.amount' + itemClass, numeral$1(item.amount).format('0,0.00 $')));
	        }
	    }
	...
	function budgetListView (budget, categories, currentDate) {
	    budget = budget || []
	    currentDate = currentDate || moment$1().format('YYYY-MM')

	    let header
	    let filtered = budget.filter(function(item) {
	        return moment$1(item.date).format('YYYY-MM') === currentDate
	    })
	    let result = []
	    ...
	    if (filtered.length > 0) {
	        result = filtered.map(function(item) {
	            ...
	            result.push(budgetItemView(title, item, currentDate))
	            return result
	        })
	        ...
	        if (total > 0) {
	            result.unshift(budgetItemView('Gesamt', {
	                amount: total
	            }))
	        }
	    } else {
	...
	m.route(document.getElementById('app'), '/', {
	    '/': {
	        controller: function() {
	            ...
	            scope.currentDate = moment$1().format('YYYY-MM')
	            scope.item = budget.create()
	        ...
	        view: function(scope) {
	            let date = moment$1(scope.currentDate, 'YYYY-MM').startOf('month')
	            ...
	            return m('div.row', [
	                ...
	                m('div.col.s12.m7.l8', [
	                    budgetListView(scope.budget(), scope.categories(), scope.currentDate),
	                    navigation
	                ])
	            ])
	        }
